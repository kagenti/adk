...
In Kagenti ADK, you communicate with the client by yielding data. This concept consists of two parts:
...
The Kagenti ADK allows you to yield simple Python types. These are automatically converted into textual A2A `Message` objects for ease of use.
...
yield AgentArtifact(
        parts=[
            Part(text="Here is the documentation you requested."),
            Part(
...
<Warning>
While yielding plain A2A components is supported, the Kagenti ADK is optimized for convenience wrappers. Using wrappers ensures the best UX in the GUI.
</Warning>
...
async def example_agent():
    yield Message(role=Role.ROLE_AGENT, message_id=str(uuid.uuid4()), parts=[Part(text="Hello from the agent!")])
```
...
# @server.agent(...)
async def example_agent():
    yield Part(text="Hello from the agent!")
```
...
async def example_agent():
    yield Artifact(
        ...
        parts=[Part(url="https://www.ibm.com/us-en", media_type="text/html", filename="IBM Website")],
    )
```
...
```python
import uuid
from a2a.types import TaskStatus, Message, TaskState, Part, Role

# @server.agent(...)
async def example_agent():
    yield TaskStatus(
        message=Message(message_id=str(uuid.uuid4()), role=Role.ROLE_AGENT, parts=[Part(text="Please provide some input.")]),
