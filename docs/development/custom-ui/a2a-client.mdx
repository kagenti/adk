...
To communicate with agents, you use the A2A protocol client to send messages and receive streaming events. The Kagenti ADK SDK provides both the A2A client transport and helpers that turn stream events into UI updates.

If you only need the fast path, start with **[Getting Started](./getting-started)**.

## Prereqs

- **Packages installed**: `@kagenti/adk` and `eventsource-parser`
...
- Create the A2A client with `buildAgentClient({ baseUrl, providerId, token })` and start a message stream.
...
const responseStream = client.sendMessageStream({
  message: {
    ...
    contextId: context.id,
    taskId,
    parts: [{ text: "Here are the project details." }],
...
for await (const event of responseStream) {
  if ("statusUpdate" in event && event.statusUpdate?.status?.state === "canceled") {
    renderCanceledState();
    break;
  }
...
Tasks can fail at any point during streaming. Failed or rejected updates arrive as `statusUpdate` events with `status.state` set to `failed` or `rejected`. In those cases:
...
for await (const event of responseStream) {
  if ("statusUpdate" in event && event.statusUpdate) {
    ...
    if (state === "failed" || state === "rejected") {
      const errorMetadata = readError(event.statusUpdate.status?.message?.metadata);
