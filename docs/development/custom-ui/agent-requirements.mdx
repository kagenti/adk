...
Kagenti ADK extends the [A2A Protocol](https://a2a-protocol.org/) with
[A2A extensions](https://a2a-protocol.org/latest/topics/extensions/) that enable agents to access platform services and
...
All extension definitions are exported from `@kagenti/adk/extensions` if you prefer a narrower import surface.
...
```typescript
import { MCPTransportType, handleAgentCard } from '@kagenti/adk';
...
```typescript
import { extractUiExtensionData, formRequestExtension } from '@kagenti/adk';
...
```typescript
import { approvalExtension, extractUiExtensionData } from '@kagenti/adk';
...
```typescript
import { canvasExtension, extractUiExtensionData } from '@kagenti/adk';
...
```typescript
import { citationExtension, extractUiExtensionData } from '@kagenti/adk';
...
```typescript
import { extractUiExtensionData, trajectoryExtension } from '@kagenti/adk';
...
```typescript
import { agentDetailExtension, extractUiExtensionData } from '@kagenti/adk';
...
```typescript
import { extractUiExtensionData, oauthRequestExtension } from '@kagenti/adk';
...
```typescript
import { extractUiExtensionData, secretsRequestExtension } from '@kagenti/adk';
...
```typescript
import { handleTaskStatusUpdate, TaskStatusUpdateType } from '@kagenti/adk';

for await (const event of stream) {
  if ("statusUpdate" in event && event.statusUpdate) {
    handleTaskStatusUpdate(event.statusUpdate).forEach((result) => {
      switch (result.type) {
