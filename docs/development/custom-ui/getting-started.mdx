...
2. A platform API client that talks to the Kagenti ADK server.
...
```bash
npm i @kagenti/adk eventsource-parser
```

### 2. Create the platform API client

```typescript
import { buildApiClient, createAuthenticatedFetch } from "@kagenti/adk";

const baseUrl = "https://your-adk-instance.com";
...
<Tip>
  **Obtaining the access token**
  ...
  For React SPAs, [`react-oidc-context`](https://github.com/authts/react-oidc-context) handles the full OIDC flow including token refresh (`automaticSilentRenew`) and discovery. Wrap your app in `AuthProvider` and use `useAuth()` to get the access token. For non-React apps, implement the Authorization Code + PKCE flow against your identity provider's discovery endpoint.
</Tip>
...
Use `buildAgentClient` to create a fully configured A2A client. It handles authenticated fetch, agent card fetching, extension discovery, and JSON-RPC + SSE transport setup.
For a full walkthrough (status updates, user responses, and cancellation), see **[A2A Client Integration](./a2a-client)**.

```typescript
import { buildAgentClient, handleAgentCard } from "@kagenti/adk";
...
`buildAgentClient` uses `createAuthenticatedFetch` internally to attach the `Authorization` header to every request.

<Warning>
  **Do not use `@a2a-js/sdk` `ClientFactory`**

  The Kagenti ADK platform proxies A2A requests and serves agent cards without the `url` field that `@a2a-js/sdk` requires for transport negotiation. Use `buildAgentClient` (or the lower-level `createA2AClient` + `fetchAgentCard`) instead.
</Warning>
...
graph TB
    ...
    subgraph Kagenti ADK [Kagenti ADK Platform]
        Agent[Agent]
        API[Platform API]
    end
...
The SDK exports A2A protocol types and Zod schemas that match the Kagenti ADK UI usage, including:
