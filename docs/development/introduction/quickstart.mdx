...
```bash
uv python install --quiet --python-preference=only-managed --no-bin 3.14 && uv tool install --python-preference=only-managed --python=3.14 --refresh --force --prerelease allow kagenti-cli && kagenti-adk self install
```
