...
The Server SDK is a Python library that enhances your existing AI agents with platform capabilities. Whether you've built your agent with LangGraph, CrewAI, or custom logic, the SDK connects it to the Kagenti ADK platform, giving you instant access to runtime-configurable services, interactive UI components, and deployment infrastructure.
...
<Tip>
**Using an AI coding assistant?** The [Kagenti ADK wrapper skill](https://github.com/kagenti/adk/tree/main/skills/kagenti-adk-wrapper) can automatically adapt your existing agent to run on the platform. Install it with:
```bash
npx skills add kagenti/adk --skill kagenti-adk-wrapper
```
</Tip>
...
Kagenti ADK utilizes A2A extensions to extend the protocol with Kagenti ADK-specific capabilities. They enable your agent to access platform services and enhance the user interface beyond what the base A2A protocol provides.

There are two types of extensions:

#### UI Extensions

UI extensions add extra metadata to messages, enabling the Kagenti ADK UI to render more advanced interactive components:
