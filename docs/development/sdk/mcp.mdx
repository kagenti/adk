...
description: "Learn how Kagenti ADK helps with authentication when using MCP."
---

## What are Connectors?

A connector is an MCP endpoint exposed by the Kagenti ADK that forwards MCP requests to a remote MCP server, acting as a proxy and handling authentication.
...
Users can work with connectors in two ways via the Kagenti ADK UI:
...
One of the key features of Kagenti ADK's MCP integration is automatic connector matching. When you specify a suggested connector name in your agent, the platform automatically:
...
<Tip>
This GitHub MCP Agent uses the connector-based approach. It requires pre-configuration of a GitHub connector.
For information on pre-configuration, see [Experimental/MCP Connectors](../experimental/connectors).
</Tip>

Here's an example of how to setup the GitHub connector using the Kagenti ADK (use your own personal access token where noted in the example code):

```python
import asyncio
from kagenti_adk.platform import Connector, ConnectorState
...
Here's an example agent that uses the GitHub MCP Connector and returns your profile details:
...
Use `mcp_service.create_client()` to get a connected MCP client.
</Step>

<Step title="Use MCP tools.">
Use an MCP client session to call tools, list resources, or interact with prompts.
</Step>
</Steps>
...
Unlike the connector-based approach, this method does not require users to set up a connection ahead of the conversation. The OAuth extension handles the authentication flow through the Kagenti ADK UI, allowing users to authorize your agent to access third-party services during the conversation itself. This makes it ideal for scenarios where you want users to authenticate on-demand without pre-configuration.
